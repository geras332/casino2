...
const server = require("https").createServer({
    key: fs.readFileSync('/etc/letsencrypt/live/testkaz.site/privkey.pem'),
    cert: fs.readFileSync('/etc/letsencrypt/live/testkaz.site/fullchain.pem')
}),
io = require("socket.io")(server, {
    cors: {
        origin: "https://testkaz.site",
        methods: ["GET", "POST"]
    }
});
...
async function promoPublish(time){
    requestify.request(domain+'/chat/promo/publish34', {
        method: 'POST'
...
function sendPromo() {
	requestify.post(domain+'/chat/promo/publish34')
	.then(function(res) {
