...
nodeCron.schedule('0 13 * * *', async () => {
    setTimeout(async () => {
        request.post('https://testkaz.site/createPromoTGok').then(function(response) {
...
📢 Сайт работает в штатном режиме, выводы в среднем до 2 часов.`, {
                ...
                inline_keyboard: [
                    [
                        { "text": "Активировать промокод", "url": "https://testkaz.site/" }
                    ]
                ]
                })
            })
...
nodeCron.schedule('0 18 * * *', async () => {
    setTimeout(async () => {
        request.post('https://testkaz.site/createPromoTG').then(function(response) {
            const res = response.getBody();
            return bot.sendMessage("@demosoyou", `
💰 Промокод 10₽/250акт — ${res.promoSum}
